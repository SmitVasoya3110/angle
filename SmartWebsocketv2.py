...
class SmartWebSocketV2(object):
    ...
    def _on_open(self, wsapp):
        ...
        self.current_retry_attempt = 0  # Reset retry counter on successful connection
        
        if self.RESUBSCRIBE_FLAG:
            print("Resubscribing to previous subscriptions...")
            self.resubscribe()
        else:
            print("First time connection, calling on_open...")
    ...
    def _on_error(self, wsapp, error):
        ...
        if self.current_retry_attempt < self.MAX_RETRY_ATTEMPT and not self.CLOSE_CONNECTION:
            print(f"Attempting to reconnect (attempt {self.current_retry_attempt + 1} of {self.MAX_RETRY_ATTEMPT})...")
